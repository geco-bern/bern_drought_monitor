...
# MeteoSwiss data:
if (TRUE) { # NOTE: activate this for automatic update
...
p_pt <- ggplot() +
  ...
  geom_text(data = meteo_pcwd_PT |> filter(year %in% highlight_years) |>
    ...
    aes(x = doy, y = PCWD_PT, colour = as.factor(year), label = year),
...
p_thorn <- ggplot() +
  ...
  geom_text(data = meteo_pcwd_THORNTWAITE |> filter(year %in% highlight_years) |>
    ...
    aes(x = doy, y = PCWD_THORN, colour = as.factor(year), label = year),
    vjust = -0.1, show.legend = FALSE
  ) +
  geom_text(data = meteo_pcwd_THORNTWAITE |> filter(!year %in% highlight_years) |>
      group_by(year) |>
    ...
    aes(x = doy, y = PCWD_THORN, label = year),
...
Error sources such as clouds can potentially lead to anomalous values. 
Note that the above map is automatically generated and has not necessarily been checked visually.
Below we show a snapshot from July 16th 2026, that appears to be cloud-free:

```{r fig.cap=paste('swissEO Vegetation Health Index around Bern (dating from 2026-07-16).')}
#| label: fig-map-vhi_fixedDate_2026-07-16
#| fig-width: 8
#| fig-height: 5
