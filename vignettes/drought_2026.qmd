...
The soil has dried out much earlier and more strongly in 2026 than in 2025. The current levels of the soil water potential (low values indicate dry conditions) are lower than the driest values in 2025. Also, the low values were attained much earlier in the season.
...
```{r}
#| label: fig-twd
#| fig-cap: "Seasonal dynamics of tree water deficit at different landscape positions (above Aare river) in 2025 and 2026. Precipitation from nearby MeteoSwiss station Bern/Zollikofen is shown for reference."
#| fig-width: 8
#| fig-height: 5
...
# Extend above base plot with precipitation
precip_threshold <- 0.1 # mm/day only plot events larger than this

df_TWD_blog_precip <- meteo_pcwd |>
  ...
  mutate(
    ...
    # Define hardcode baseline for precipitation plotting:
    y_max_P = ifelse(year == 2025, 550, 500)
  ) |>
  select(date, year, plot_date, P, y_max_P)
p_twd_blog +
  geom_rect(
    data = df_TWD_blog_precip |> filter(P > precip_threshold),
    mapping = aes(
      x = plot_date,
      ymax = y_max_P, ymin = y_max_P - P,
      fill = year
    ),
    width = 1, y = NULL, group = NA, show.legend = FALSE
  ) +
  labs(
    title = "Tree water deficit across individual trees",
    x = NULL,
    y = "Tree water deficit (\u00b5m) and Precip (mm/day)"
  )
```

**Finding**

Tree water deficits are as high or higher than ever recorded in 2025. However, in contrast to 2025, deficits have started rising much earlier. This indicates that water stress is has been affecting trees throughout almost the full growing season this year. In contrast, water stress effects at the high and mid positions were limited to August in 2025. This likely has important negative consequences for the carbon balance and growth of trees in 2026.

The low position appears to be better buffered against drought impacts in both years, with the exception of one individual tree.
...
The SwissSMEX monitoring network provides continuous observations of soil moisture at multiple depths across Switzerland. Current conditions can be explored through the official monitoring platform:
